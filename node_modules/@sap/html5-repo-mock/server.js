        ...
        if (manifest["sap.app"].crossNavigation &&
            ...
            return Object.values(manifest["sap.app"].crossNavigation.inbounds).map(inbound => {
                return {
                    ...
                    title: translate(i18n, inbound.title) || translate(i18n, manifest['sap.app'].title) || (meta.applicationName + '-' + meta.applicationVersion),
                    properties: {
                        chipId: "catalogTile_" + meta.applicationId + '_' + meta.applicationVersion,
                        title: translate(i18n, inbound.title || manifest['sap.app'].title) || (meta.applicationName + '-' + meta.applicationVersion),
                        subtitle: translate(i18n, inbound.subTitle || manifest['sap.app'].subTitle || manifest['sap.app'].description),
                        icon: inbound.icon || (manifest['sap.ui'] && manifest['sap.ui'].icons && manifest['sap.ui'].icons.icon) || "sap-icon://product",
...
function interpolate(placeholder, xml) {
    if(xml && placeholder && placeholder.startsWith('${project.') && placeholder.endsWith('}')) {
        const tag = placeholder.slice(2, placeholder.length-1).split('.').pop();
...
function getBusinessService(sapCloudService) {
    return {
        "binding_name": null,
        "credentials": {
            "html5-apps-repo": {
        ...
        "instance_name": sapCloudService,
        "label": sapCloudService,
        "name": sapCloudService,
        "plan": "standard",
        "provider": null,
        "syslog_drain_url": null,
        "tags": [ sapCloudService, "mock-business-service" ],
        "volume_mounts": []
    };
}
...
module.exports.bootstrap = async function bootstrap({dir, baseUrl, relative, ui5, APIKey, inject, mimeTypes}) {
    ...
    // Base directories in which to look for HTML5 applications
    let baseDirs;
    if(Array.isArray(dir)) {
        baseDirs = dir;
    } else {
        baseDirs = (await fs.readdir(dir, { withFileTypes: true })).reduce((acc, cur) => {
            if(cur.isDirectory()) {
                acc.push(path.join(dir, cur.name))
            }
            return acc;
        }, [dir]);
    }
    ...
    const metadata = (await Promise.all(baseDirs.map(async (baseDir) => {
        try {
            await fs.access(path.join(baseDir, 'xs-app.json'));
            let pom;
            try {
                pom = await fs.readFile(path.join(baseDir, 'pom.xml'), 'utf8');
            } catch (err) {
                pom = '';
            }
            for(let i=0; i<lookupDirs.length; i++) {
                ...
                try {
                    ...
                    const appId = interpolate(manifest['sap.app'].id, pom);
                    const appName = appId.replace(/[.\-]/g, '');
                    const appVersion = interpolate(manifest['sap.app'].applicationVersion.version, pom).replace('-SNAPSHOT','');
                    ...
                    if (sapCloudService) {
                        ...
                        VCAP_SERVICES[serviceName] = [getBusinessService(sapCloudService)];
                        console.log(`\t${appId}@${appVersion}\t->\t${baseUrl}${serviceName}.${appName}-${appVersion}/`);
                    }
                    console.log(`\t${appId}@${appVersion}\t->\t${baseUrl}${appName}-${appVersion}/`);
                    return {
                        applicationId: appId,
    ...
    .get('/portal/xs-app.json', res => {
        res.writeHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({
            "authenticationMethod": VCAP_SERVICES.xsuaa ? "route" : "none",
            "routes": [{
                "source": "^(.*)",
                "target": "$1",
