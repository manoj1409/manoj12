...
function overrideDependencies() {
    ...
    if((flpFlagIndex = process.argv.indexOf('--flp')) >= 0) {
        if(standaloneFlagIndex >= 0) {
            console.error(`Configuration error. --flp and --standalone flags can't be used together!`)
            process.exit(1);
        }
...
function arrayOrString(arg) {
    if(typeof arg === 'string') {
        if(arg[0] === '[' && arg[arg.length-1] === ']') {
            ...
            return arg.split(',').map(s => s.trim()).filter(s => !!s)
        }
        return arg;
    }
    return '';
...
mockServer.bootstrap(
    {
        // HTML5 apps root
        dir: arrayOrString(process.env.MOCK_DIR) || path.resolve(process.cwd(), '..'),
        // Base URL
    ...
    if(welcomeFile) {
        ...
        // Add header, when request is to service instance destination
